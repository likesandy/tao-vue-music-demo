...
export const getLetterArtists = (letter) => {
  return new Promise(function (resolve, reject) {
    let letterArtists = []
    Network.all([
      Network.get(`artist/list?limit=2&type=-1&area=7&initial=${letter}`),
      Network.get(`artist/list?limit=2&type=-1&area=96&initial=${letter}`),
      Network.get(`artist/list?limit=1&type=-1&area=16&initial=${letter}`)
    ])
      .then(function (result) {
        // console.log(result)
